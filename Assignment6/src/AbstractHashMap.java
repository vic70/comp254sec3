...
public abstract class AbstractHashMap<K,V> extends AbstractMap<K,V> {
  ...
  public AbstractHashMap(int cap, int p) {
    this(cap, p, 0.5);  //default load factor
  }
  ...
  /** Crates a hashtable with optional load factor, capacity 17 and prime factor 109345121.*/
...
//      resize(2 * capacity - 1);        // (or find a nearby prime)
    if (n > capacity *loadfactor)              // variable load factor
      resize( (int) Math.ceil(capacity/loadfactor - 1));        // (or find a nearby prime)
    return answer;
  }
