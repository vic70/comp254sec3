...
//  /** Bottom-up approach of Merge-sort of array S. */
//  public static <K> void mergeSortBottomUp(K[] S, Comparator<K> comp) {
//    int n = S.length;
//    if (n < 2) return;                        // array is trivially sorted
//    // divide
//    int mid = n/2;
...
//  }
  ...
  public static <K> void mergeSortBottomUp(Queue<K> S, Comparator<K> comp){
    ...
    Queue<Queue<K>> queueOfS = new LinkedQueue<>();

    //place each item in a queue
    for (int i = 0; i <n; i++){
      Queue<K> s_Single = new LinkedQueue<>();
      s_Single.enqueue(S.dequeue());
      queueOfS.enqueue(s_Single);
    }


    while (queueOfS.size() != 1){   //merge until all queue merged into a single queue
      Queue<Queue<K>> TempQueueOfS = new LinkedQueue<>();

      //Merge all queues
      while(!queueOfS.isEmpty()){
        Queue<K> S1 = queueOfS.dequeue();
        Queue<K> S2 = queueOfS.isEmpty()? new LinkedQueue<>(): queueOfS.dequeue(); //Empty queue number of queue if odd

        Queue<K> mergedS = new LinkedQueue<>();

        merge(S1,S2,mergedS,comp);  //merge the queue

        TempQueueOfS.enqueue(mergedS); //put the merged queue into a temp queue of queue
      };

      queueOfS = TempQueueOfS;  //put all merged queue back for another cycle of merge
    }

    Queue<K> sortS = queueOfS.dequeue();

    while (!sortS.isEmpty()){
      S.enqueue(sortS.dequeue());
    }
  }
