 ...
 */
public class Ex1_TreeMap<K,V> extends AbstractSortedMap<K,V> {
   ...
   */
  public Ex1_TreeMap(Comparator<K> comp) {
  ...
  private Position<Entry<K,V>> treeSearchWithLoop(Position<Entry<K,V>> p, K key) {
    ...
    while (!isExternal(current)) {
      ...
      else if (compare(key, current.getElement()) < 0)
      {
    ...
    return current;   // key not found; return the final leaf
  }
  public V getWithNewTreeSearch(K key) throws IllegalArgumentException {
    checkKey(key);                          // may throw IllegalArgumentException
    Position<Entry<K,V>> p = treeSearchWithLoop(root(), key);
    rebalanceAccess(p);                     // hook for balanced tree subclasses
    if (isExternal(p)) return null;         // unsuccessful search
    return p.getElement().getValue();       // match found
  }
  ...
  public static void main (String[] args)
  {
	  ...
	  Ex1_TreeMap<Integer,String> map = new Ex1_TreeMap<Integer,String>();
	  map.put(6, "A");
	  map.put(2, "B");
	  map.put(4, "C");
	  map.put(1, "D");
	  ...
	  map.put(8, "F");
	  //search
	  System.out.println("Get(4) using original treeSearch with recursion:");
      System.out.println(map.get(4));
      System.out.println("Get(4) using new treeSearch without recursion:");
      System.out.println(map.getWithNewTreeSearch(4));
